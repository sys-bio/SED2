...
- **`ParameterScan`'s `outputs.json` dimensions stay plain English**, not the `expr`/`valid` notation defined in Section 8: "one dimension per entry in `parameterRanges`" needs an iteration construct (one dimension per entry of a collection, each sized by a per-entry formula) that notation doesn't have yet. `Scatter`'s and `ExplicitODESimulation`'s own `range.numberOfSteps`/`independentVariableRange.numberOfSteps` formulas have the same latent gap the conditional form just closed for `NumericRange`/`ParameterRange` themselves - `numberOfSteps` may be absent when the referenced range was given `values` directly instead - noted today rather than formalized.

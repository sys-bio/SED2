...
Two grammars compose: expressions (the math itself) and references (the `#...` addressing syntax embedded inside them). An EBNF sketch follows, with evidence-level noted inline - `attested` productions come directly from SED2_2026_extracted.txt's own text and worked examples; `proposed` productions have no textual precedent and are flagged as open in core-spec.md Section 10 until confirmed.

```
(* --- expressions (attested except where marked) --- *)
expression      ::= logicalOr
logicalOr       ::= logicalAnd ( ("or" | "xor") logicalAnd )*        (* proposed *)
logicalAnd      ::= logicalNot ( "and" logicalNot )*                  (* proposed *)
logicalNot      ::= "not" logicalNot | relational                     (* proposed *)
relational      ::= additive ( ("==" | "!=" | "<" | ">" | "<=" | ">=") additive )?   (* proposed *)
...
atom            ::= NUMBER
                   | REFERENCE
                   | IDENTIFIER "(" arglist? ")"      (* function or distrib call *)
                   | IDENTIFIER                        (* bare constant: pi, true, ... *)
                   | "[" arglist? "]"                  (* array literal *)
                   | "(" expression ")"
                   | "piecewise" "(" arglist ")"       (* proposed - see note below *)
...
For `piecewise`: no example anywhere in the source text uses it, so the call shape above - alternating `(condition, value)` pairs with an optional trailing unpaired default value - is a placeholder proposal, not a transcription of anything attested. Same for the relational/logical operators' concrete spellings (`==`, `and`, ...): the Calculation section's own "Allowed operations" list names only `+`, `-`, `*`, `/`, `^`, parentheses, PEMDAS, and "the functions/constants allowed in SBML" - which, read as the full SBML Level 3 Core MathML subset, categorically does include relational/logical/piecewise vocabulary, but whether SED2's own infix syntax was meant to expose it, and with which spelling, is unconfirmed. Both are tracked in core-spec.md Section 10 pending a real decision; `schema/predefined-functions.json` marks every such entry `"attested": false` so the generator (or a future stricter grammar) can distinguish confirmed vocabulary from placeholder.
...
| `{location}` | the failing value's JSON-pointer-style instance path, for a catch-all rule when it doesn't cleanly name a single `{attr}` |

...
* Java: `libsed2j` - libsbml's own Java bindings build a jar named `libsbmlj.jar` (a suffix `j`, not a prefix), and `libsed2j` mirrors that; Maven `artifactId`: `libsed2j`. The `groupId` is still open - libsbml and JSBML both use a domain-backed groupId (`org.sbml`) this project doesn't have yet. `io.github.<org>` is the standard placeholder Sonatype recommends for a project with no registered domain, but the actual value depends on where the repository ends up hosted, so it's left as a TBD here rather than guessed.

**Build tools.**

* C++: CMake, with `FetchContent` pulling nlohmann::json (ordered_json), pboettch/json-schema-validator, GoogleTest, and the ANTLR4 C++ runtime (see Math's Parser Strategy) directly from their upstream repositories - no vendored copies, no package-manager layer (Conan/vcpkg) in front of them.
...
`IDENTIFIER` follows the existing `SId` pattern (`specsheets/core/Types/v1.0.0/schema.json`): `^[A-Za-z_][A-Za-z0-9_]*$`. A reference's first `IDENTIFIER` must name one of `SEDDocument`'s own ID-keyed children (`tasks`, `outputs`, `constants` currently - `styles` once it's more than a placeholder); every following colon-delimited `IDENTIFIER` walks one level down that element's own containment tree (`#tasks:repeat:subTasks:sim1`); `.IDENTIFIER` accesses a named non-structural output a task declares in its own `outputs.json` (`.model`, `.strings`, `.aggregates`, ...); `[index]` indexes into AnnotatedData, positionally (`[3]`), by label (`['S1']`), by negative (Python-style, from-the-end) index (`[-1]`), or by range (`[2:5]`, `[-3:-1]`) - matching `core/Types`'s `SIdRef` pattern, which already accepts all four forms. `hasSubvalue()` (see Cross-references) validates a `subaccess*` chain against the referenced task's `outputs.json` without needing to run anything.
...
### Validation
The JSON schema pass rejects structurally malformed documents (wrong types, missing required fields, pattern mismatches) with a human-readable message, but the schema-validator libraries themselves don't expose a reliable way to map a failure back to a stable rule number - pboettch in particular reports only the instance location and a free-text message, with no schema-location or constraint-keyword information (a pull request adding a keyword field is still open and unmerged, and its author declined to add schema-location tracking at all) - so numbering can't be built on top of the schema libraries' own error output directly (see Schema-Pass Errors below for how validate() numbers them anyway).
